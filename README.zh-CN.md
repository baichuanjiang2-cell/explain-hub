...
```text
我的 GitHub 收藏 · 73 个仓库
├── A 级 26 个 → 深度讲解：README（三角度 + file:line 锚点）
│                + 功能全景图 + 模块架构图 + 运行流程图
└── B 级 47 个 → 一页纸轻量讲解

本机已装技能 · 55+ 个（软链接农场去重后）
...
从一次真实的批量讲解工程（35 篇技能讲解 + 73 个仓库讲解 + 78 张图表，
全部通过独立视觉验收）沉淀而来。装进 Claude Code、Codex、ZCode 或任何
兼容 Agent Skills 的宿主，然后对你的 agent 说话即可。
...
一次真实运行：73 个 stars、26 个 A 级项目、78 张图表全部通过独立视觉验收；
35 个本机技能全量讲解。本仓库的 `references/` 就是那次运行踩坑经验的沉淀。
